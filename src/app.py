...
def load_data() -> pd.DataFrame:
    try:
        raw_app = os.getenv("DATABRICKS_APP_NAME", "")
        if not raw_app:
            raise RuntimeError("DATABRICKS_APP_NAME not set in environment.")
        catalog_name = raw_app[:-4] if raw_app.lower().endswith("-app") else raw_app
